...
async function performDatabaseMaintenance() {
    try {
        ...
        for (const idx of indexes) {
            if (idx.unique && idx.name !== 'idNumber_1_ministry_1' && idx.key.idNumber === 1 && Object.keys(idx.key).length === 1) {
        ...
        await collection.createIndex(
            { idNumber: 1, ministry: 1 },
            { unique: true, name: 'idNumber_1_ministry_1' }
        );

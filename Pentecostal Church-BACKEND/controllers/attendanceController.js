...
exports.signAnonymous = async (req, res) => {
    try {
        let { sessionId, yearJoined, name, idNumber, gender, ageGroup, phoneNumber, phone, signature, userType } = req.body;

        // Support both field names for phone
        const effectiveIdNumber = idNumber;
        ...
        if (userType === 'student' && !effectiveIdNumber) {
            return res.status(400).json({ message: 'ID number is required for members' });
        }
        ...
        const idNumberToCheck = (effectiveIdNumber || 'N/A').trim().toUpperCase();
        console.log(`[signAnonymous] Signing for session: ${session.title} (${sessionId}), User: ${name}, ID No: ${idNumberToCheck}`);

        // Only check for duplicates if an ID number is provided
        if (effectiveIdNumber) {
            const existingRecord = await AttendanceRecord.findOne({ sessionId, idNumber: idNumberToCheck });
            if (existingRecord) {
                return res.status(400).json({
                    message: `ID number ${idNumberToCheck} has already signed attendance for this session`
                });
            }
        }
        ...
        const attendanceRecord = new AttendanceRecord({
            sessionId,
            userName: name.trim(),
            idNumber: idNumberToCheck,
            gender: gender || 'Other',
            ageGroup: ageGroup || 'Adult',
            userType: userType || 'student',
            yearJoined: yearJoined || session.ministry || 'General',
        ...
        if (error.code === 11000) {
            return res.status(400).json({ message: 'ID number already signed' });
        }

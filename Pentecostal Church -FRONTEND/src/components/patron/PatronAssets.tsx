...
const PatronAssets: React.FC = () => {
    ...
    const ensureFinanceToken = async () => {
        ...
        if (token) return; // already have one
        try {
            const res = await fetch('/api/finance/auth/login', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email: 'patron@rpc.ac.ke', password: 'Patron@2026' }),
            });
            ...
            // silent — fetchAssets will show the error if still no token
        }
    };

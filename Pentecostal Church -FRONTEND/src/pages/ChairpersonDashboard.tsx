...
const ChairpersonDashboard: React.FC = () => {
    ...
    const fetchUsers = async (showLoader = false) => {
        ...
        userData.forEach((user: User) => {
            if (user.yos) groupedByYos[user.ageGroup] = (groupedByYos[user.ageGroup] || 0) + 1;
        ...
        setUsersByAgeGroup(groupedByYos);
        setUsersByMinistry(groupedByMinistry);
        setUsersByEt(groupedByEt);

        // Calculate participation
        const inMin = userData.filter((u: User) => [].length > 0).length;
        const inEt = userData.filter((u: User) => [].length > 0).length;
    ...
    const handleExportPdf = () => {
        try {
            ...
            sortedYosKeys.forEach(yos => {
                ...
                const title = `RPC Members List - Year ${yos}`;
                ...
                for (let i = 1; i <= pageCount; i++) {
                    ...
                    doc.text('Generated by RPC Nyamira Chairperson Dashboard', 15, pageHeight - 10);
                    doc.text(`Page ${i} of ${pageCount}`, doc.internal.pageSize.width - 15, pageHeight - 10, { align: 'right' });
                }

                doc.save(`RPC_Members_Year_${yos}_${new Date().toISOString().split('T')[0]}.pdf`);
            });
        } catch (error) {
    ...
    const WelcomeSplash = () => (
        <div style={{
            ...
            <div style={{ padding: '40px', background: 'rgba(255,255,255,0.1)', borderRadius: '24px', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.2)' }}>
                ...
                <h1 style={{ fontSize: '32px', fontWeight: '800', marginBottom: '8px', letterSpacing: '-0.5px' }}>Welcome RPC Nyamira Chairperson</h1>
    ...
    const renderMembers = () => {
        ...
        const displayUsers = users.filter(user => {
            ...
            const matchAge = isEtView ? user.ageGroup === activeFilterText : true;
    ...
    return (
        <div className={styles.pageWrapper} style={{
            ...
            <footer style={{
                ...
                &copy; {new Date().getFullYear()} RPC Nyamira Chairperson Portal
            </footer>
        </div>
    );
};

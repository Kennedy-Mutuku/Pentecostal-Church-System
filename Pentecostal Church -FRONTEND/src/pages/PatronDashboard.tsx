...
const PatronDashboard: React.FC = () => {
    ...
    const fetchUsers = async (showLoader = false) => {
        ...
        userData.forEach((user: User) => {
            if (user.ageGroup) {
                groupedByAgeGroup[user.ageGroup] = (groupedByAgeGroup[user.ageGroup] || 0) + 1;
            }
        });
        ...
        if (lastCounts.current.users > 0 && userData.length > lastCounts.current.users) {
            ...
            addNotification(
                ...
                `${newCount} new Member(s) joined. Latest: ${latest.username}`,
                'user',
                'members'
            );
        }
    ...
    // PDF export grouped by Age Group
        ...
        try {
            ...
            sortedAgeGroupKeys.forEach(ageGroup => {
                ...
                for (let i = 1; i <= pageCount; i++) {
                    ...
                    doc.text('Generated by RPC Nyamira Patron Dashboard', 15, pageHeight - 10);
            ...
            <div style={{ 
                ...
                <div style={{ position: 'relative', display: 'inline-block', marginBottom: '30px' }}>
                    ...
                    <div style={{
                        ...
                        borderTopColor: '#FFB300', borderRightColor: '#FFB300',
                ...
                <h1 style={{ 
                    ...
                    Welcome Rikuruma Pentecostal Church Nyamira Senior Pastor
                </h1>
                
                <h2 style={{ 
                    fontSize: '26px', fontWeight: '600', color: '#FFB300', 
                    letterSpacing: '1px', textShadow: '0 2px 10px rgba(255,179,0,0.3)',
                ...
                <div style={{ marginTop: '40px', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', animation: 'slideUpFade 0.8s ease-out 0.6s both' }}>
                    <div style={{ width: '200px', height: '4px', background: 'rgba(255,255,255,0.1)', borderRadius: '4px', overflow: 'hidden', position: 'relative' }}>
                        <div style={{ 
                    ...
                    <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '3px', fontWeight: '600' }}>
                        Initializing Portal
                    </span>
                </div>
            </div>
        ...
        <div className={styles.section} style={{ ...sectionStyle, padding: '0 24px 24px' }}>
            ...
            <div style={{ 
                ...
                {[
                    ...
                    {
                      val: users.filter(u => u.gender === 'Male').length,
                      label: 'Male Members',
                      sub: userCount > 0 ? `${((users.filter(u => u.gender === 'Male').length / userCount) * 100).toFixed(1)}% of total` : '0% of total',
                      icon: <User size={16} />,
                      bg: 'linear-gradient(135deg, #1d4ed8, #1e40af)'
                    },
                    {
                      val: users.filter(u => u.gender === 'Female').length,
                      label: 'Female Members',
                      sub: userCount > 0 ? `${((users.filter(u => u.gender === 'Female').length / userCount) * 100).toFixed(1)}% of total` : '0% of total',
                      icon: <User size={16} />,
        ...
        return (
            <>
                ...
                <div className={styles.section} style={{ ...sectionStyle, padding: '0 16px 16px' }}>
                    ...
                    {/* Search + Age Group filter + Refresh */}
    ...
    return (
        ...
        <div className={styles.adminLayout} style={{ background: '#f5f5f5' }}>
            ...
            <footer style={{
                ...
                &copy; {new Date().getFullYear()} RPC Nyamira Senior Pastor Portal
            </footer>
        </div>
    );
};
